...
%	\newpage
	...
	Note that if we want to know the shortest paths from $s$ to other vertices with exactly $1 \le k \le m$ edges, instead of $v.d^\prime$ we can use $v.d[0..|G.V| - 1]$. So $v.d[i]$ tell us the shortest path from $s$ to $i$ with exactly $i$ edges:
	\begin{algorithm*}[h!]
		\begin{algorithmic}[1]
