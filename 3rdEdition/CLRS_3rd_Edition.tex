...
%	\newpage
	...
	The running time of matrix-list implementation is $O(V^3)$. For analyzing the running time of adjacency-list implementation we can use amortized analysis.
	\begin{enumerate}
	...
	\begin{split}
		\sum_{u \in V}{in_u} &= |E| \\
		\sum_{u \in V}{out_u} &= |E| \\
		\sum_{(u,v) \in E} {in_u} &\le \sum_{u \in V} {in_u}
	\end{split}	
	\end{equation*}
	Note that $\sum_{(u,v) \in E} {in_u} \le \sum_{u \in V} {in_u}$ is correct because there maybe at least one vertex $u$ in which $in_u >0 \land out_u = 0$.
	\begin{equation*}
		\begin{split}
