...
%	\newpage
	...
	\end{enumerate}
	We assign to each edge (u, v) cost $c_{(u, v)} = 1 + in_u$. Because by traversing all vertices (line \ref{MakeSquareGraph:all_verticies}), we finally visit $u$ and  then visit (u, v) in line \ref{MakeSquareGraph:1-path} which occurs exactly 1 time. For each edge that enters $u$ we \textbf{revisit} edge (u, v) (lines \ref{MakeSquareGraph:2-path:1} - \ref{MakeSquareGraph:2-path:2}). We know that $\sum_{u \in V}{in_u} + \sum_{u \in V}{out_u} = 2|E|$. So we can easily calculate the total cost.
	\begin{equation*}
		\begin{split}
		\sum_{(u, v) \in E}{c_{(u, v)}} &= \sum_{(u, v) \in E}{1 + in_u} \\
		&= \sum_{(u, v) \in E}{1} + \sum_{(u, v) \in E}{in_u} \\
		&\le |E| + |E| \times \sum_{u \in V}{in_u} \\
		&\le 4|E|
		\end{split}
	\end{equation*}
	We execute line \ref{MakeSquareGraph:1-path} at most $|E|$ times and lines \ref{MakeSquareGraph:2-path:1} - \ref{MakeSquareGraph:2-path:2} at most $3|E|$ times. So the total running time of algorithm using adjacency-list is $O(|V| + 4|E|) = O(V + E)$.
	\begin{algorithm*}[h!]
				...
				\While{$Q \ne \emptyset$}
					...
					\ForAll{$v \in G.adj[u]$}
						\If{$v.color == WHITE$} \Comment edge $(u, v)$ is a tree edge
							...
							\State $v.\pi = u$
							\State \Call{Enqueue}{Q, v}
						\ElsIf{u.d == v.d + 1} \Comment v.color is Black
							\State continue							
						\ElsIf{v.d == u.d + 1} \Comment v.color is GRAY (cross edge)
							\State Continue
						\ElsIf{u.d == v.d} \Comment v.color is GRAY (cross edge)
							\State \Return $FALSE$
