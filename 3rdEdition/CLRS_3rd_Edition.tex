...
%	\newpage
			...
			\Function{BFS}{G, s}
				...
				\While{$Q \ne \emptyset$}
					...
					\ForAll{$v \in G.adj[u]$}
						...
						\ElsIf{v.d == u.d + 1} \Comment back edge
							\State \Call{Assert}{$v.color == GRAY$}
						\ElsIf{u.d == v.d} \Comment back edge
							\State \Call{Assert}{$v.color == GRAY$}
	...
	For more information about Lemma 22.3 see the textbook. Suppose $Q = \{v_1, v_2, \dots , v_r\}$ as we defined it in Lemma 22.3. Note that if we encounter a back edge, it connects two vertices (u, v) that $u \in Q \land v \in Q$. To be more specific, first we pop $u$ from queue and then we find out $(u, v)$ is a back edge. In that moment $u \notin Q \land v \in Q$.\\
	
	Suppose $(u, v)$ is a back edge. According to Lemma 22.3 from the textbook, $u.d \le v.d \land v.d \le u.d + 1$. If $v.d = u.d + 1$, then $u$ and $v$ have different colors. So we only need to consider $u.d = v.d$. In that case both $u$ and $v$ have the same color and we need to prove that this graph cannot be bipartite. When we have a back edge it means that we have a cycle (see figure \ref{fig:DFS tree}). The number of edges in this cycle is $u.d + v.d + 1 = 2 \times u.d + 1$ which is odd. So the graph cannot be bipartite. Note that the graph can have more than one connected component so it is possible we need to run BFS more than once.
	\begin{algorithm*}[h!]
				...
				\While{$Q \ne \emptyset$}
					...
					\ForAll{$v \in G.adj[u]$}
						...
						\ElsIf{u.d == v.d + 1} \Comment v.color is Black
							\State continue							
						\ElsIf{v.d == u.d + 1} \Comment v.color is GRAY (back edge)
							\State Continue
						\ElsIf{u.d == v.d} \Comment v.color is GRAY (back edge)
							\State \Return $FALSE$
	...
	Note that a directed graph is acyclic if and only if a depth-first search yields no back edges. Undirected graphs are tricky. Since $(u, v) \in E \land (v, u) \in E$, $(u, v)$ is a back edge if and only if $(v, u)$ is a tree edge.
