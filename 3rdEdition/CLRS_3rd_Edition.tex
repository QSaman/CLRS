...
%	\newpage
	...
	For a simple $O(V^2)$ implementation of Prim algorithm see exercise 2. Also see Dijkstra shortest path algorithm.
	\paragraph{Exercise 2}
	...
	\begin{algorithm*}[h!]
		\begin{algorithmic}[1]
			\Function{BELLMAN-FORD2}{G, w, s}
				\State \Call{INITIALIZE-SINGLE-SOURCE}{G, s}
				\For{$m = 1$ \textbf{ to } $|G.V| - 1$} \Comment{shortest path with exactly $m$ edges}
					\State $updated = FALSE$
	...
	\begin{algorithm*}[h!]
		\begin{algorithmic}[1]
			\Function{BELLMAN-FORD3}{G, w, s}
				\State \Call{INITIALIZE-SINGLE-SOURCE}{G, s}
				\For{$m = 1$ \textbf{ to } $|G.V| - 1$} \Comment{shortest path with exactly $m$ edges}
					\State $updated = FALSE$
					\ForAll{edge $(u, v) \in G.E$}
						\If{\Call{RELAX}{u, v, w, m} $ == TRUE$}
							\State $updated = TRUE$
						\EndIf
					\EndFor
					\If{updated $ == FALSE$}
		...
		\end{algorithmic}
			...
			\begin{algorithmic}[1]
				\Function{RELAX}{u, v, w, m}
					\If{$v.d[m] > u.d[m - 1] + w(u, v)$}
	...
	\begin{algorithm*}[h!]
		\begin{algorithmic}[1]
			\Function{BELLMAN-FORD4}{G, w, src, dst, y}
				\State \Call{INITIALIZE-SINGLE-SOURCE}{G, src}
				\For{$m = 1$ \textbf{ to } $y$} \Comment{shortest path with exactly $m$ edges}
					\State $updated = FALSE$
					\ForAll{$v \in G.V$}
						\State $v.d^\prime = \infty$
					\EndFor
	...
	Note that by running Strongly connected components on a normal graph we can convert it to a DAG.
	\par Since there is not cycle, we are not worried about negative cycles but it's ok if we have at least one edge with negative weight.
	\par First we sort all vertices in topological sort order. So if there is a path between $s$ (the root of shortest-path tree) to $u$, We visit $s$ before $u$. So we start from the first vertex in the topological sorted order and relax all its children. We do the same for the next vertex. The running time of this algorithm is $O(V + E)$.
	\begin{algorithm*}[h!]
		\begin{algorithmic}[1]
			\Function{Initialize-Single-Source}{$G,s$}
				\ForAll{$v \in G.V$}
		...
		\begin{algorithmic}[1]
			\Function{DAG-SHORTEST-PATHS}{$G,w,s$}
				\State Topological sort the vertices in $G$
				\State \Call{Initialize-Single-Source}{G,s}
	...
	\section{Dijkstra's algorithm}		
		\begin{algorithm*}[h!]
			...
			\begin{algorithmic}[1]
				\Function{Dijkstra}{$G,w,s$}
					\State \Call{Initialize-Single-Source}{G,s}
					\State $S = \emptyset$
					\State $Q = G.V$ \Comment{heapify: $O(V)$}
					\While{$Q \ne \emptyset$} \Comment{$O(V)$}
						\State $u=$ \Call{Extract-Min}{Q} \Comment{$O(\log{V})$ or $O(V)$}
						\State $S = S \cup \{u\}$
						\ForAll{$v \in G.adv[u]$} \Comment{$\sum_{v \in V}{outdegree(v)} = O(E)$}
							\If{\Call{Relax}{u, v, w} == TRUE}
								\State \Call{Decrease-Key}{Q, v, v.d} \Comment{$O(\log{V})$ or $O(1)$}
							\EndIf
						\EndFor
					\EndWhile
				\EndFunction
		...
		\end{algorithm*}
		\FloatBarrier
	As you can see if we implement Dijkstra algorithm using binary heap, converting the array to a heap (heapify) takes $O(V)$. At the beginning the size of $Q$ is $|V|$ and in each iteration we extracting the minimum. So it takes $O(V\log{V})$. Decreasing the key (up-heapifying)  is done at most $E$ times ($\sum_{v \in V}{outdegree(v)} = E$). So it takes $O(E\log{V})$. So the total running time is $O(V\log{V} + E\log{V})$.
	\par If we use a heap like Fibonacci or Brodal heap which takes $O(1)$ for decreasing the key, the total running time is $O(V\log{V}) + O(E)$. For more information about heaps see this \href{https://en.wikipedia.org/wiki/Heap_(data_structure)}{page}.
	\par Instead of heap we can use a normal array. So it takes $O(V)$ to populate the array. Finding the minimum takes $O(V)$. So EXTRACT-MIN takes $O(V^2)$. DECREASE-KEY takes $O(1)$. So the total running time of this implementation is $O(V^2) + O(E)$. If $E = O(V^2)$ then this implementation has the same performance as Fibonacci implementation:
	\begin{algorithm}[h!]
		\caption{$O(V^2) + O(E)$ Dijkstra shortest path}		
		\begin{algorithmic}[1]
			\Function{Dijkstra}{$G,w,s$}
				\State \Call{Initialize-Single-Source}{$G,s$}
				\For {$i = 1$ to $|G.V|$} \Comment{$O(V)$}
					\State $min = \infty$
					\ForAll{$w \in G.V$} \Comment{$O(V)$}
						\If{$w.mark == FALSE \land w.d < min$}
							\State $min = w.d$
							\State $u = w$
						\EndIf
					\EndFor
					\State $u.mark = TRUE$
					\ForAll{$v \in G.adj[u]$} \Comment{$\sum_{v \in V}{outdegree(v)} = O(E)$}
						\If{$v.mark == FALSE$}
							\State \Call{Relax}{$u, v, w$}
						\EndIf
					\EndFor
	...
	\end{algorithm}
	\FloatBarrier
	In most languages like std::priority\_queue in C++, we don't have operation Decrease-Key. We can insert an edge instead of a vertex. More precisely $(u, v).d = v.d = u.d + w(u, v0)$. We define function $INSERT(PQ, v, (u, v).d)$ that insert edge $(u, v)$ into the priority queue. The key in priority queue is $(u, v).d$, so $Extract-Min(PQ)$ returns $v$.
	\par In this case extracting the minimum takes $O(E\log{E})$. Inserting into priority queue which is equivalent to Decrease-Key takes also $O(E\log{E})$. So the total running time of this implementation is $O(E\log{E})$. So the total running time of this algorithm is $O(E\log{E}) = O(E\log{V^2}) = O(2E\log{V}) = O(E\log{V})$. If $E = O(V^2)$ this implementation is worse than the previous one. But for maximum flow implementation this implementation has better running time.
	
    \begin{algorithm*}[h!]
		\caption{$O(E\log{V})$ Dijkstra shortest path}    	        
        \begin{algorithmic}[1]
            \Function{Dijkstra}{$G,w,s$}
                \State \Call{Initialize-Single-Source}{G,s}
                \State $S = \emptyset$
                \State \Call{INSERT}{PQ, s, 0}
                \State $visited = 0$
                \While{$PQ \ne \emptyset \land visited < |G.V|$} \Comment{$O(E)$}
                    \State $u=$ \Call{Extract-Min}{PQ} \Comment{$O(\log{E})$}
                    \If{$u \in S$}
                    	\State continue
                    \EndIf
                    ...
                    \State $S = S \cup \{u\}$               
                    \ForAll{$v \in G.adv[u]$} \Comment{$\sum_{v \in V}{outdegree(v)} = O(E)$}
                    	\If{$v \notin S \land \Call{Relax}{$u, v, w$}$}
                    		\State \Call{INSERT}{Q, v, v.d} \Comment{$O(\log{E})$}
                        \EndIf
                    \EndFor
                \EndWhile
            \EndFunction
		...
		\begin{enumerate}
			...
			\begin{itemize}
				\item[\textbf{Solution 1:}] We can use an algorithm similar to Bellman-Ford shortest-path. The running time of this algorithm is $O(VE)$. We define $cnt[i][j]$ as the number of paths from $s$ to $i$ with exactly $j$ edges. As you can see we can improve space complexity by using $cnt^{\prime}[1..|V|][1..2]$, instead of $cnt[1..|V|][0..e]$. But for simplicity we use $cnt$.
                    \begin{algorithm}[h!]
                        \begin{algorithmic}[1]
                            \Function{Initialize-Single-Source}{$G,s, d, e$}
                                \ForAll{$v = 1$ to $|G.V|$}
                                	\ForAll{$i = 1$ to $e$}
                                		\State $cnt[v][i] = 0$
                                	\EndFor
                        ...
                        \begin{algorithmic}[1]
                            \Function{Relax}{$u, v, d, i$}
 	                            \State $cnt[v][e] += cnt[u][e - 1]$
                            \EndFunction
                        \end{algorithmic}                    
                        \begin{algorithmic}[1]
                            \Function{Bellman-Ford}{$G,s, d, e$}
                            	\State Let $cnt[1..|V|][0..e]$ be a new array
                                \State \Call{Initialize-Single-Source}{G,s, d, e}
                                \For{$i = 1$ TO $e$} \Comment{Finding shortest paths with exactly $i$ edges}
                                    \ForAll{$(u, v) \in G.E$}
                                        \State \Call{Relax}{u, v, d, i}
                                    \EndFor
                                \EndFor
				...
				\item[\textbf{Solution 2:}] You can use DFS but without marking the previously visited vertices. The running time of this algorithm is $O(V^k)$. For more information visit this \href{https://www.geeksforgeeks.org/count-possible-paths-source-destination-exactly-k-edges/}{website}.
				\par There is a better solution using an algorithm similar to Floyd-Warshall shortest past algorithm. We define $cnt[i][j][v]$ as the number of all paths from $i$ to $j$ which have exactly $v$ vertices as intermediate. In other words a path like $i, v_{i_1}, v_{i_2}, \dots, v_{i_v}, j$. So if we want to find the number of paths between $s$ and $d$ with exactly $e$ edges, we need to find $cnt[s][d][e - 1]$. So we define adjacency matrix $A=(a_{ij})$ as:
				\begin{equation*}
				a_{ij} = \begin{cases}
					0 & i = j \lor (i, j) \notin E\\
					1 & i \ne j \land (i, j) \in E
				...
				\begin{algorithm}[h!]
					\begin{algorithmic}[1]
						\State \Function{Count-Paths}{A, s, d, e}
							\State $n = A.rows$
							\State let $cnt[1..n][1..n][e - 1]$ be a new array
							\ForAll{$i = 1$ to n}
								\ForAll{$j = 1$ to n}
									\State $cnt[i][j][0] = A[i][j]$
								\EndFor
							\EndFor
							\ForAll{$v = 1$ to $e - 1$}
								\ForAll{$i = 1$ to $n$}
									\ForAll{$j = 1$ to $n$}
										\State $cnt[i][j][v] = 0$
										\ForAll{$k = 1$ to $n$}
											\If{$A[i][k] \ne 0 \land k \ne j$} \Comment{We know $A[i][i] = 0$}
												\State $cnt[i][j][v] += cnt[k][j][v - 1]$
											\EndIf
										\EndFor
									\EndFor
								\EndFor
			...
			\end{itemize}
			\phantomsection
			\label{psec:shortest_path_with_exactly_k_edges}
			\item Shortest path with exactly k edges in a directed and weighted graph
				\begin{itemize}
					\item[\textbf{Solution 1:}] For more information visit this \href{https://www.geeksforgeeks.org/shortest-path-with-exactly-k-edges-in-a-directed-and-weighted-graph-set-2/?ref=rp}{page}.
                    ...
                    \begin{algorithm}[h!]
                        \begin{algorithmic}[1]
                            \Function{Initialize-Single-Source}{$G,s, d, k$}
                                \ForAll{$v = 1$ to $|G.V|$}
                                	\ForAll{$i = 1$ to $k$}
                                		\State $d[v][i] = \infty$
                                	\EndFor
                        ...
                        \begin{algorithmic}[1]
                            \Function{Relax}{$u, v, w, d, e$}
                                \If{$d[v][e] > d[u][e - 1] + w(u, v)$}
                                    \State $d[v][e] = d[u][e - 1] + w(u, v)$
                                \EndIf
                            \EndFunction
                        \end{algorithmic}                    
                        \begin{algorithmic}[1]
                            \Function{Bellman-Ford}{$G,w,s, d, k$}
                            	\State Let $d[1..|V|][0..k]$ be a new array
                                \State \Call{Initialize-Single-Source}{G,s, d, k}
                                \For{$e = 1$ TO $k$} \Comment{Finding shortest paths with exactly $e$ edges}
                                    \ForAll{$(u, v) \in G.E$}
                                        \State \Call{Relax}{u, v, w, d, e}
                                    \EndFor
                                \EndFor
                    ...
                    \begin{algorithm}[h!]
                        \begin{algorithmic}[1]
                            \State \Function{Shortest-Paths}{W, s, d, e}
                                \State $n = W.rows$
	...
	\begin{algorithm*}
		\begin{algorithmic}[1]
			\Function {DFS-VISIT}{G, T, s, u}
				\State $T[s][u] = TRUE$
				\ForAll{$v \in G.adj[u]$}
					\If{$T[u][v] == FALSE$}
						\State \Call{DFS-VISIT}{G, T, s, v}				
					\EndIf
				\EndFor
			\EndFunction
		\end{algorithmic}
	...
	Suppose in flow network $G = (V, E)$ it is possible $(u, v) \in E \land (v, u) \in E$. We need to redefine some definitions. Consider Ford-Fulkerson algorithm. In each step we increase flow $f$ until we reach maximum flow. Suppose the flow of $G$ is $f_i$ in i\textsuperscript{th} step. So we have $f_1, f_2, \dots, f_m$. It is obvious $|f_1| = 0$ and $|f_m|$ is the value of maximum flow.
	\par Suppose the network has $|f_{i - 1}|$ and $p$ is an augmenting path from source to sink. We define \textbf{residual capacity} of $p$ as $c_{f_i}(p) = \min\{c_{f_{i - 1}}(u, v) : (u, v) \text{ is on } p\}$.
	 \par Suppose we are in the i\textsuperscript{th} step and $p$ is an augmenting path from source to sink. The \textbf{residual capacity} in the i\textsuperscript{th} step is given by
	\begin{equation}
	\label{eq:ResidualCapacityWithAntiParallel}
		...
		\end{cases}
	\end{equation}
	For the base case we have $c_{f_1} = C$ in which $C$ is the capacities for all edges. 
	\par Now consider the Ford-Fulkerson Algorithm. For $(u, v) \in G.E$, $(u, v).c$ is the capacity of that edge. We don't change it during the iteration. $(u,v).c_f$ is residual capacity of edge $(u, v)$ which changes during the execution. $(u, v).f$ is the current flow in $(u, v)$. If it's negative it means the actual flow is from $v$ to $u$. So $(u, v).f = -(v, u).f$. Also we assume if $(u, v) \in G.E$ then $(v, u) \in E$. $v.minCap$ is the minimum $c_f$ in the chosen path from $s$ to $t$. 
	...
	\begin{algorithm*}[h!]
		\begin{algorithmic}[1]
			\Function{Ford-Fulkerson}{G, s, t}
				...
				\EndFor
				\While{$TRUE$}						
					\State minCap = \Call{FIND-AUGMENTING-PATH}{G, s, t}
					\If{$minCap = 0$}
						\State break
					\EndIf
					\State $v = t$
					\While{$v \ne NIL$}
						\State $u = v.\pi$
						\State $(u , v).c_f = (u , v).c_f - minCap$
						\State $(u, v).f = (u, v).f + minCap$ 
						\State $(v, u).c_f = (v, u).c_f + minCap$
						\State $(v, u).f = (v, u).f - minCap$
						\State $v = u$
					\EndWhile
				\EndWhile
			\EndFunction
	...
	\end{algorithm*}
	\FloatBarrier
	Implementation function FIND-AUGMENTING-PATH can be trick. If we use BFS, the algorithm is called Edmonds-Karp. The running time of that algorithm is $O(VE^2)$. If we choose the path poorly (for example using DFS instead of BFS) the algorithm many never exit.
	...
	\begin{algorithm*}[h!]
		...
		\begin{algorithmic}[1]		
			\Function{FIND-AUGMENTING-PATH}{G, s, t}
				\State \Call{Initialize-Single-Source}{$G,s$}
				\State $s.color = GRAY$
				\State $s.\pi = NIL$
				\State $Q = \emptyset$
				...
				\While{$Q \ne \emptyset$}
					...
					\EndIf
					\ForAll{$v \in G.adj[u]$}
						\If{$v.color == WHITE \land (u, v).c_f > 0$} 
							\State $v.color = GRAY$
							\State $v.\pi = u$
							\State $v.minCap = \min(v.minCap, (u, v).c_f)$
							\State \Call{Enqueue}{Q, v}
						\EndIf
					\EndFor
					\State $u.color = BLACK$
	...
	\end{algorithm*}		
	\FloatBarrier
	\par Instead of queue, we can use a priority queue. This implementation is similar to Dijkstra shortest path algorithm. Function INSERT(Q, vertex, key) insert $(vertex, key)$ into priority queue. Function Extract-Max(Q) extracts the vertex in which $key$ is maximum. We use $(u, v).c_f$ as key. Note that if indegree vertex $v$ is more than 1, then it's possible we inserted $v$ more than once. In other words the maximum size of priority queue is $|G.E|$. Finding an augmenting path using the following implementation takes $O(E\log{E}) = O(E\log{V})$.
    ...
    \begin{algorithm*}[h!]
        ...
        \begin{algorithmic}[1]
            \Function{FIND-AUGMENTING-PATH}{G, s, t}
                \State \Call{Initialize-Single-Source}{G,s}
                \State $S = \emptyset$
                \State \Call{INSERT}{Q, s, $\infty$}
                \State $found = FALSE$
                \State $visited = 0$
                \While{$Q \ne \emptyset \land visited < |G.V|$} \Comment{$O(E)$}
                    \State $u=$ \Call{Extract-Max}{Q} \Comment{$O(\log{E})$}
                    \If{$u \in S$}
                    	\State continue
                    \EndIf
                    \State $visited = visited + 1$
                    ...
                    \ForAll{$v \in G.adv[u]$} \Comment{$\sum_{v \in V}{outdegree(v)} = O(E)$}
                    	\If{$(u, v).c_f > 0 \land v \notin S$}
                    		\State $v.\pi = u$
                    		\State $v.minCap = \min(v.minCap, (u, v).c_f)$
                    		\State \Call{INSERT}{Q, v, $(u, v).c_f$} \Comment{$O(\log{E})$}
                        \EndIf
                    \EndFor
                \EndWhile
                \If{$found == TRUE$}
    ...
    \end{algorithm*}
    \FloatBarrier	
	\par If we want to use an implementation similar to $O(V^2 + E)$ in Dijkstra shortest path, then finding an augmenting path takes $O(E^2 + E)$:
	...
	\begin{algorithm}[h!]
		\begin{algorithmic}[1]
			\Function{FIND-AUGMENTING-PATH}{G, s, t}
				...
				\State $found = FALSE$
				\For {$i = 1$ to $|G.E|$} \Comment{$O(E)$}
					\State $max = 0$
					\ForAll{$(x, y) \in G.E$} \Comment{$O(E)$}
						\If{$(x, y).mark == FALSE \land (x, y).d > max$}
							\State $max = (x, y).d$
							\State $(p, u) = (x, y)$
						\EndIf
					\EndFor
					...
					\EndIf
					\ForAll{$v \in G.adj[u]$} \Comment{$\sum_{v \in V}{outdegree(v)} = O(E)$}
						\If{$(u, v).mark == FALSE \land (u, v).c_f > 0$}
							\State $v.\pi = u$
							\State $v.minCap = \min(v.minCap, (u, v).c_f)$
							\State $(u, v).d = (u, v).c_f$
						\EndIf
		...
		\begin{algorithmic}[1]
		\Function{Initialize-Single-Source}{$G,s$}
			\State $G.E = G.E \cup (s, s)$
			\ForAll{$(u, v) \in G.E$} \Comment{$O(E)$}
				\State $(u, v).d = 0$
				\State $(u,v).mark = FALSE$
			\EndFor
			\State $(s, s).d = \infty$
			\ForAll{$v \in G.V$}
				\State $v.\pi = NIL$
